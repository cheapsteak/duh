...
GitHub gist via `gh gist create --secret`, and returns the viewer URL
...
def test_share_creates_gist_and_returns_viewer_url(gist_server, scanned):
    ...
    resp = json.load(_get(f"{base}/api/share/{root['id']}"))
...
def test_share_unknown_node_404(gist_server):
    ...
    base, _ = gist_server
    with pytest.raises(urllib.error.HTTPError) as e:
        _get(f"{base}/api/share/999999999")
...
@rust_only
def test_share_gh_failure_returns_502(scanned):
    """A `gh` that exits nonzero (e.g. not signed in, API error) must map to
    502 with the trimmed stderr in the error message."""
    import urllib.error
    proc, base = _start_server(scanned, {"DUH_GH_BIN": str(STUB), "DUH_STUB_FAIL": "1"})
    try:
        root = json.load(_get(f"{base}/api/root"))
        with pytest.raises(urllib.error.HTTPError) as e:
            _get(f"{base}/api/share/{root['id']}")
